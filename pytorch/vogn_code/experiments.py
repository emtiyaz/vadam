...
class ExperimentBBBMLPBinClass(Experiment):
    ...
    def __init__(self, data_set, model_params, train_params, optim_params, evals_per_epoch=1, normalize_x=False, results_folder="./results", data_folder=DEFAULT_DATA_FOLDER, use_cuda=torch.cuda.is_available()):
        ...
        def objective(logits_list, y):
            return metrics.avneg_elbo_bernoulli(logits_list, y, train_set_size = self.data.get_train_size(), kl = self.model.kl_divergence())
    ...
    def _evaluate_model(self, metric_dict, x_train, y_train, x_test, y_test):
        ...
        metric_dict['elbo_neg_ave'].append(metrics.avneg_elbo_bernoulli(logits_list, y_train, train_set_size = self.data.get_train_size(), kl = self.model.kl_divergence()).detach().cpu().item())
...
class ExperimentVadamMLPBinClass(Experiment):
